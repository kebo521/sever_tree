...
//=============================统计字符种类======================================================
u16 CountStringDifferentNumber(u16 MaxNum,const char* pStringGroup[], DfStringOffset *pOffset,DfTagCharUnit *pCountTable)
{
	u16 i=0,num=0;
	DfTagCharUnit Tag,hChar;
	Tag.B2Char=0xffff;
	while(i<MaxNum)
	{
		hChar.B1Char[0]=pStringGroup[pOffset[i].pY][pOffset[i].pX];
		...
		if(Tag.B2Char != hChar.B2Char)
		{
			Tag.B2Char = hChar.B2Char;
			num++;
		}
		//-----建立统计表--------
		if(pCountTable) pCountTable[i].B2Char= Tag.B2Char;
...
//================创建空间树形链节点===========================================================
DfTreeNode* CreateTreeLink(const char* pString,void *pfuntion)
{
	DfTreeNode *pStart=NULL,*pNext;
	u16 dh;
	while(*pString)
	{
		dh=*pString++;
		if(dh>0x80) dh = dh*0x100 + *pString++;	//支持汉字
		if(pStart)
		{
			pNext->pFilial = (DfTreeNode*)gfixMalloc();
			pNext=pNext->pFilial;
		}
		else 
		{
			pNext=(DfTreeNode*)gfixMalloc();
			pStart=pNext;
		}
		memset(pNext,0x00,sizeof(DfTreeNode));
		pNext->B2Char = dh;
	}
	if(pStart) pNext->pfun = pfuntion;
	return pStart;
}
	
DfTreeNode* LoadTreeLink(DfTreeNode *pStart,DfTreeNode *pAddLink)
{
	DfTreeNode *pNext,*p;
	if(pStart==NULL) return pAddLink;
	pNext=pStart;
	while(pNext)
	{
		if(pNext->B2Char == pAddLink->B2Char)
		{
			p=pAddLink;
			pAddLink=pAddLink->pFilial;
			if(pAddLink == NULL)
			{
				pNext->pfun= p->pfun;
				gfixFree(pAddLink);
		...
		else if(pNext->pBrother)
		{
			pNext=pNext->pBrother;
		}
		else
		{
			pNext->pBrother=pAddLink;
			return pStart;
		}
	}
	return pStart;
...
DfBuffTreeNode* FindStringBuffNode(const char* pTargetChars ,u16 CharsLen,DfBuffTreeNode *pStardNode)
{
	...
	while(Offset < CharsLen)
	{
		...
		for(;;)
		{
			...
			{
				if(pStardNode->btu[middle].Next == NULL)
				{
...
DfTreeNode *StructureTransformDispe(DfBuffTreeNode *pStardNode)
{
	...
	for(i=0;i<pStardNode->Number;i++)
	{
		if(pTreeNode ==NULL)
		{
			pTreeNode=(DfTreeNode *)gfixMalloc(sizeof(DfTreeNode));
			pStard=pTreeNode;
		}
		else
		{
			pTreeNode->pBrother=(DfTreeNode *)gfixMalloc(sizeof(DfTreeNode));
...
int TreeDemoNode1(void)
{
	...
	TRACE_HEX("TreeBuff:",pStardNode,sizeof(pStardNode->Len)+pStardNode->Len);
...
int TreeDemoNode2(void)
{
	...
	while(1)
	{
		...
		{
			gets(sExplain);
			//scanf_s("%s", sExplain,sizeof(sExplain));
			AddFlag=0;
		}
	}
